...
I use two IP pools. One IP pool for dynamically allocated IPs for applications and one for statically assigned IPs. You really won't need many static IPs in the static pool. I only use them for my home lab DNS services when need an IP. Once you install the external-dns plugin, Kubernetes will dynamically add DNS records for the services you deploy.
...
I chose OpenSuSE Leap as my base OS, which is in the enterprise Linux vein, since K3s is related to or a product of Rancher (also produces Longhorn distributed storage), and Rancher which is "by" SuSE...Which OpenSuse Leap is the open-source upstream distro of SUSE Enterprise Linux (SLES).
